      ...
      {filteredBlogs.map((blog, i) => {
        ...
        return (
          <a
            key={blog.id}
            href={`/blogs/${blog.id}`}
            className={clsx("flex aspect-[4/3]", isBig && "md:col-span-2 md:row-span-2")}
          >
...
const Pages = () => {
  return (
    <div className="relative grid flex-1 grid-cols-2 md:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-7 gap-3">
      <Page page={1} />
    </div>
  );
};
